...
const initSocket = (server, corsOptions) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("ADMIN_ACCEPT", (userId) => {
      ...
      // Perform actions when admin accepts user request
      // You can add code here to log the user out or perform any other necessary actions
      io.emit("USER_LOGOUT", userId, {
        message: "Please login Again",
      });
    });
    socket.on("ADMIN_DECLINE", (userId) => {
      // Perform actions when admin accepts user request
      // You can add code here to log the user out or perform any other necessary actions
      io.emit("USER_LOGOUT", userId, {
        message: "Please Contact Support Admin Decline Your Request",
      });
    });
      ...
      invitedUser.forEach((invitedUser) => {
        ...
        if (socketId) {
          socket.to(socketId).emit("INVITED_TO_ROOM", {
            message: `${creator} has added you to the ${name} chat room.`,
          });
        }
      });
    });
